...
| [includes](#includes) | 关联表数据的查询会默认从列的字段中获取，如果显示的时候没有配置，则可以提供额外的关联表字段配置。规则和 `parse` 的 `include` 一致。 | Array  | false |
...
| 属性值   | 描述                                       |
| ----- | ---------------------------------------- |
| web   | 执行的是由 `plugin` 配置提供函数，`action` 的值必须与 `plugins` 提供的函数名一致 |
